...
const storage = new MMKV({ id: 'bbc-offline-queue' })
const QUEUE_KEY = 'pending_events'
const ERROR_KEY = 'error_events'
...
function readErrors(): QueuedEvent[] {
  try {
    const raw = storage.getString(ERROR_KEY)
    return raw ? (JSON.parse(raw) as QueuedEvent[]) : []
  } catch {
    return []
  }
...
function writeErrors(items: QueuedEvent[]): void {
  storage.set(ERROR_KEY, JSON.stringify(items))
}
  ...
  try {
    ...
    for (const item of all) {
      ...
      try {
        ...
        // 2xx: success — drop item (not pushed to remaining)
      } catch {
        remaining.push({ ...item, retries: item.retries + 1 })
        networkFailed = true
      }
